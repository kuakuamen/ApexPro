...
<div class="bg-gray-800/50 backdrop-blur-sm border border-gray-700 shadow-lg overflow-hidden sm:rounded-lg max-w-4xl mx-auto">
    ...
    <div class="p-6">
        ...
        <form id="measurementForm" action="{{ route('personal.measurements.update', $measurement) }}" method="POST" enctype="multipart/form-data" data-student-url="{{ route('personal.students.show', $student) }}">
            ...
            <div x-data="measureComparisonEdit()" x-init="initTracking()" class="mb-6">
                    ...
                    <div x-show="open"
                        ...
                        <div class="px-5 pb-5 border-t border-indigo-500/20">
                            ...
                            @php
                                ...
                                $hasAnyCircEdit = collect($circFieldsEdit)->some(fn($f) => $previousMeasurement->{$f[0]} !== null);
                            @endphp
                            @if($hasAnyCircEdit)
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm">
                                    ...
                                    <tbody class="divide-y divide-slate-800/50">
                                        @foreach($circFieldsEdit as [$fld, $lbl, $digGood, $neut])
...
<script>
    ...
    function measureComparisonEdit() {
        const prev = @json($previousMeasurement ?? null);
        const current = @json($measurement);
        const numericFields = [
            'weight','body_fat','muscle_mass','chest','waist','abdomen','hips',
            'right_arm','left_arm','right_thigh','left_thigh','right_calf','left_calf',
        ];
        const initialCurr = {};
        numericFields.forEach(f => {
            const v = parseFloat(String(current[f] ?? '').replace(',', '.'));
            if (!isNaN(v)) initialCurr[f] = v;
        });
        return {
            open: true,
            curr: initialCurr,
