...
@section('content')
<div class="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl shadow-lg max-w-4xl mx-auto">
    <div class="px-4 py-5 border-b border-gray-700 sm:px-6">
        <h3 class="text-lg leading-6 font-medium text-white">
            Nova Avaliação Física: {{ $student->name }}
        </h3>
        <p class="mt-1 max-w-2xl text-sm text-gray-300">
            Preencha as medidas do aluno.
        </p>
    </div>
    ...
    <div class="p-6">
        ...
        <form id="measurementForm" action="{{ route('personal.measurements.store', $student) }}" method="POST" enctype="multipart/form-data" data-student-url="{{ route('personal.students.show', $student) }}">
            @csrf
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div>
                    ...
                    <input type="number" step="0.01" name="weight" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-indigo-500 focus:border-indigo-500">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-400">Altura (m)</label>
                    <input type="number" step="0.01" name="height" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-indigo-500 focus:border-indigo-500">
                </div>
                <div>
            ...
            <div id="skinfolds_container" class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6" x-data="{ 
                ...
                <div>
                    ...
                    <input type="number" step="0.1" name="tricipital" x-model="tricipital" @input="calculateSum()" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-400">Bicipital (mm)</label>
                    <input type="number" step="0.1" name="bicipital" x-model="bicipital" @input="calculateSum()" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-400">Torácica (mm)</label>
                    <input type="number" step="0.1" name="toracica" x-model="toracica" @input="calculateSum()" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-400">Abdominal (mm)</label>
                    <input type="number" step="0.1" name="abdominal_fold" x-model="abdominal_fold" @input="calculateSum()" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-400">Axilar Média (mm)</label>
                    <input type="number" step="0.1" name="axilar_media" x-model="axilar_media" @input="calculateSum()" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-400">Supra-ilíaca (mm)</label>
                    <input type="number" step="0.1" name="suprailiaca" x-model="suprailiaca" @input="calculateSum()" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-400">Coxa (mm)</label>
                    <input type="number" step="0.1" name="coxa_fold" x-model="coxa_fold" @input="calculateSum()" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-400">Panturrilha (mm)</label>
                    <input type="number" step="0.1" name="panturrilha_fold" x-model="panturrilha_fold" @input="calculateSum()" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-400">Somatório de Dobras (mm)</label>
                    <input type="number" step="0.1" name="sum_skinfolds" id="sum_skinfolds" class="mt-1 block w-full bg-gray-700/50 border border-indigo-500 rounded-lg px-4 py-3 text-indigo-300 focus:ring-indigo-500 focus:border-indigo-500" readonly>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div>
                    ...
                    <input type="number" step="0.1" name="right_forearm" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white">
                </div>
                <div>
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" rows="3" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-indigo-500 focus:border-indigo-500" placeholder="Observações gerais sobre a avaliação..."></textarea>
            </div>

            <!-- Anamnese desta Avaliação -->
            <div class="border-t border-gray-700 pt-6 mb-6" x-data="{ showAnamnese: false }">
                <div class="flex items-center justify-between mb-4 cursor-pointer" @click="showAnamnese = !showAnamnese">
                    <h4 class="text-lg font-medium text-gray-200 flex items-center">
                ...
                <div x-show="showAnamnese" class="bg-gray-800/50 p-4 rounded-lg grid grid-cols-1 md:grid-cols-2 gap-4" style="display: none;">
                    <div>
                        <label class="block text-sm font-medium text-gray-300">Histórico de Lesões</label>
                        <input type="text" name="injuries" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ex: Joelho, Ombro...">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-300">Medicamentos</label>
                        <input type="text" name="medications" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ex: Nenhum">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-300">Cirurgias</label>
                        <input type="text" name="surgeries" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ex: Apêndice (2019)...">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-300">Dores Atuais</label>
                        <input type="text" name="pain_points" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ex: Dor na lombar ao agachar...">
                    </div>
                    <div class="col-span-1 md:col-span-2">
                        <label class="block text-sm font-medium text-gray-300">Hábitos (Fumo, Álcool, Sono)</label>
                        <input type="text" name="habits" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ex: Fuma socialmente, dorme 6h/noite...">
                    </div>
                    <div class="col-span-1 md:col-span-2">
                        <label class="block text-sm font-medium text-gray-300">Objetivo Específico desta Fase</label>
                        <input type="text" name="goal" class="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ex: Focar em perder barriga para o casamento...">
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end">
                <a href="{{ route('personal.students.show', $student) }}" class="mr-3 inline-flex items-center px-4 py-2 border border-gray-600 shadow-sm text-sm font-medium rounded-lg text-gray-300 bg-gray-800 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-indigo-500 transition-all">
                    Cancelar
                </a>
...
<script>
    ...
    function showProtocolSelector() {
        ...
        if (!studentBirthDate) {
            alert('O aluno não possui data de nascimento cadastrada. Por favor, atualize o perfil do aluno.');
            return;
        }
    ...
    function calculateAllProtocols(weight, age, gender, skinfolds) {
        ...
        if (gender && (gender.toLowerCase().includes('masc') || gender.toLowerCase() === 'm' || gender.toLowerCase() === 'male')) {
            ...
            throw new Error('Gênero do aluno não está definido. Por favor, atualize o perfil do aluno.');
        }
    ...
    function calculateGuedes(weight, age, gender, skinfolds) {
        ...
        const density = isMale 
            ...
            : 1.16055 - (0.06706 * Math.log10(sum));
    ...
    function calculatePollock7Male(weight, age, skinfolds) {
        const sum = skinfolds.subescapular + skinfolds.tricipital + skinfolds.toracica + 
                    skinfolds.axilar_media + skinfolds.abdominal_fold + skinfolds.suprailiaca + skinfolds.coxa_fold;
        if (sum <= 0) return null;
    ...
    function confirmAndSubmit() {
        ...
        // Preencher body_fat e muscle_mass com o protocolo selecionado
