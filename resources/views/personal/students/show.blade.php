                        ...
                        <li>
                            <div class="px-4 py-4 flex items-center justify-between hover:bg-gray-700/30 transition-colors">
                                ...
                                <div class="ml-4 flex items-center gap-3">
                                    <a href="{{ route('workouts.pdf', $workout) }}" target="_blank" rel="noopener noreferrer" class="text-gray-400 hover:text-cyan-400 transition-colors" title="Exportar PDF">
                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 11v6m0 0l-2.5-2.5M12 17l2.5-2.5"/></svg>
                                    </a>
                                    <a href="{{ route('workouts.edit', $workout) }}" class="text-gray-400 hover:text-indigo-400 transition-colors" title="Editar">
                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <form method="POST" action="{{ route('workouts.toggle-active', $workout) }}" class="flex items-center">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                                title="{{ $workout->is_active ? 'Inativar treino' : 'Ativar treino' }}"
                                                class="transition-colors {{ $workout->is_active ? 'text-green-400 hover:text-red-400' : 'text-gray-500 hover:text-green-400' }}">
                                            @if($workout->is_active)
                                                {{-- Ativo: ícone de olho aberto (clica para inativar) --}}
                                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                            @else
                                                {{-- Inativo: ícone de olho fechado (clica para ativar) --}}
