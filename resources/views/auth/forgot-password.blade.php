...
<head>
    ...
    <title>ApexPro - Recuperar Senha</title>
    ...
    <div class="min-h-full flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
            <div class="flex flex-col items-center mb-8">
                ...
                <h2 class="text-center text-3xl font-extrabold tracking-tight">Recuperar senha</h2>
                <p class="mt-2 text-center text-sm text-gray-400">Informe seu e-mail para receber o link de redefinição.</p>
            </div>
        </div>
        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            <div class="bg-gray-800/50 backdrop-blur-xl border border-gray-700/50 py-10 px-6 shadow-2xl rounded-2xl sm:px-12">
                @if (session('status'))
                    <div class="rounded-lg bg-emerald-500/10 border border-emerald-500/20 p-4 mb-6 text-sm text-emerald-300">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <form class="space-y-6" action="{{ route('password.email') }}" method="POST">
                    ...
                    <a href="{{ route('login') }}" class="block text-center text-sm font-medium text-cyan-400 hover:text-cyan-300 transition-colors">
                        Voltar para o login
                    </a>
                </form>
            </div>
