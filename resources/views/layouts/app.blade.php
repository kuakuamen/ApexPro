...
<body class="{{ auth()->check() && auth()->user()->role === 'aluno' ? '' : (auth()->check() ? 'bg-personal-dark' : 'bg-stone-100') }} font-sans antialiased" style="{{ auth()->check() && auth()->user()->role === 'aluno' ? 'background:#0d0f1a;' : '' }}" x-data="{ sidebarOpen: false }">
                ...
                @if(auth()->check())
                    ...
                    @if(auth()->user()->role === 'personal')
                        ...
                        <a href="{{ route('diets.index') }}" class="group flex items-center px-3 py-2 min-h-[42px] text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('diets.*') ? 'bg-cyan-500/10 text-cyan-100 border-l-2 border-cyan-400 shadow-[inset_0_0_0_1px_rgba(34,211,238,0.22)]' : 'text-slate-300 hover:bg-slate-800/80 hover:text-slate-100' }}">
                            <svg class="w-5 h-5 mr-3 {{ request()->routeIs('diets.*') ? 'text-cyan-300' : 'text-slate-500 group-hover:text-cyan-300' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6V4m0 0a2 2 0 110 4m0-4a2 2 0 100 4M6 20h12a2 2 0 002-2V9a2 2 0 00-2-2H6a2 2 0 00-2 2v9a2 2 0 002 2zm2-7h8m-8 4h5"></path></svg>
                            Dietas
                        </a>
    ...
    <nav class="fixed bottom-0 left-0 right-0 z-50 md:hidden"
         style="background:rgba(11,18,32,0.97);border-top:1px solid rgba(51,65,85,0.45);backdrop-filter:blur(20px);padding-bottom:env(safe-area-inset-bottom);">
        <div class="flex items-center justify-around h-16">
            ...
            <a href="{{ route($item['route']) }}"
               ...
               style="color:{{ $active ? '#60a5fa' : '#475569' }};">
                <span style="font-size:22px;line-height:1;filter:{{ $active ? 'none' : 'grayscale(0.4) opacity(0.7)' }};">{{ $item['emoji'] }}</span>
                <span style="font-size:10px;font-weight:700;letter-spacing:0.02em;">{{ $item['label'] }}</span>
            </a>
            @endforeach
        </div>
    </nav>
